#requires -version 3
# 제2차 슈퍼로봇대전 단독판(별매 CD, SLPS_02406) 한글패치 v0.9.4 적용 엔진
...
$patch   = Join-Path $root 'srw2-standalone-korean-v0.9.4.xdelta'
...
try {
    Write-Host ''
    Write-Host '============================================================'
    Write-Host '   제2차 슈퍼로봇대전 단독판(별매 CD) 한글패치 v0.9.4'
    Write-Host '============================================================'
    Write-Host ''

    if (-not (Test-Path -LiteralPath $xdelta)) { Fail "xdelta.exe 가 없습니다. 패치 파일들을 한 폴더에 함께 두세요." }
    if (-not (Test-Path -LiteralPath $patch))  { Fail "srw2-standalone-korean-v0.9.4.xdelta 가 없습니다." }
